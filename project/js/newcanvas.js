...
jQuery(document).ready(function($) {
  ...
  window.retrieveCanvas = function () {
    jQuery(document).ready(function ($) {

        let propertyBtns = [$('#FillColor'), $('#StrokeColor')];

        $.ajax({
            url: 'php-files/retrivecanvas.php',
            type: 'GET',
            dataType: 'json',
            success: function (response) {
                if (response.success) {
                    var canvasState = response.canvas_state;
                    var parsedCanvasState = JSON.parse(canvasState);
                    console.log("parsedCanvasState: ", parsedCanvasState);

                    fabric.util.requestAnimFrame(function render() {
                        canvas.renderAll();
                        console.log("Canvas rendered.");

                        canvas.loadFromJSON(parsedCanvasState, function () {
                            canvas.renderAll();
                            console.log("Canvas state loaded successfully.");
                            addLayerToList();

                            // After loading canvas state, set up event listeners
                            setupEventListeners();
                        });
                    });
            ...
            error: function (xhr, status, error) {
                console.log('Error fetching canvas state data');
            }
        });

        // Function to set up event listeners after canvas state is loaded
        function setupEventListeners() {
            if (propertyBtns[0]) {
                propertyBtns[0].on('input', function () {
                    var activeObj = canvas.getActiveObject();
                    if (activeObj) { // Check if active object exists
                        activeObj.set({
                            fill: this.value
                        });
                        canvas.renderAll();
            ...
            if (propertyBtns[1]) {
                propertyBtns[1].on('input', function () {
                    var activeObj = canvas.getActiveObject();
                    if (activeObj) { // Check if active object exists
                        activeObj.set({
                            stroke: this.value
