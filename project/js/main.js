...
function startAddingText(o) {
    ...
    if (fillInput) {          //  changin the fonts color , stroke color strok width, size
        fillInput.addEventListener('input', function FillColor() {
            canvas.getActiveObject().set({
                fill: this.value,
    ...
    if (strokeInput ) {
        strokeInput.addEventListener('input', function StrokeColor() {
            canvas.getActiveObject().set({
                stroke: this.value,
            });
            saveCanvasState();
    ...
    if (fontProperty[0]) {
        fontProperty[0].addEventListener('change',function fontSize() {
    ...
    if (fontProperty[1]) {
        fontProperty[1].addEventListener('change',function FontWeight() {
    ...
    if (fontProperty[2]) {
        fontProperty[2].addEventListener('click', function Bold() {
            canvas.getActiveObject().set({
                fontStyle:'bold',
            });
            canvas.renderAll();
    ...
    if (fontProperty[3]) {
        fontProperty[3].addEventListener('click', function Italic() {
        ...
        fontProperty[3].addEventListener('dblclick',function Italic() {
...
function applyFilter() {
 ...
 /** LAYER PANEL TO CHANGE THE  EDIT AND MAKE CHANGE  */
    ...
    function addLayerToList() {
        ...
        canvas.getObjects().forEach((obj, index) => {
            ...
            icon1.src = '/pages/icons/svg/eye-open.svg';
            icon1.className = 'icon';
            icon1.addEventListener('click', (event) => {
                if (icon1.src.endsWith('/pages/icons/svg/eye-open.svg')) {
                    icon1.src = '/pages/icons/svg/eye-closed.svg';
                    obj.visible = false;
                } else {
                    icon1.src = '/pages/icons/svg/eye-open.svg'; 
                    obj.visible = true;
            ...
            // Create lock/unlock button icon
            const icon2 = document.createElement('img');
            icon2.className = 'icon';
            icon2.src = '/pages/icons/svg/lock-open.svg'; 
            icon2.addEventListener('click', (event) => {
                if (icon2.src.endsWith('/pages/icons/svg/lock-open.svg')) {
                    icon2.src = '/pages/icons/svg/lock-close.svg'; 
                    obj.selectable = false;
                    obj.hasControls=false;
                    obj.hasBorders=false;
                    canvas.selection = false; // Disable canvas selection
                    canvas.renderAll();
                    event.stopPropagation();
                } else {
                    icon2.src = '/pages/icons/svg/lock-open.svg'; 
                    ...
                    obj.hasBorders=true;                    
                    canvas.selection = true; // Enable canvas selection
                    canvas.renderAll();
                    event.stopPropagation();
                }
            });
            ...
            // Add click event listener to select object on canvas
            ...
            // Check if the current object is active, and add 'selected' class
...
function initializeSortable() {
    Sortable.create(layerList, {
        ...
        onUpdate: function(event) {
            ...
            // Clear canvas and add objects in new order
            canvas.clear();
            newObjectsOrder.forEach(obj => {
                     canvas.add(obj);
            });

            // Update the layer list to reflect the new order
            addLayerToList();
        }
    });
}
...
function removeObject(){
    var acvtiveObject=canvas.getActiveObject();
    document.addEventListener('keydown', function (e) {
        if ((e.key === 'Delete' || e.key === 'Backspace')) {
            if (acvtiveObject !== 'text') {
                canvas.remove(acvtiveObject);
                canvas.renderAll();}
                redo();
                undo();
            }
...
// COUSTOM COlOR PICKER 

// document.querySelectorAll('.FillColor').forEach(input => {
...
window.onload = retrieveCanvasData;